import React, { useEffect, useState } from 'react'
import { YOUTUBE_SEARCH_VIDEO_API } from '../utils/consts'
import { useSelector } from 'react-redux'

const Head = () => {
  const [searchText,setSearchtext]=useState("")
  const [suggestion,setSuggestion]=useState([])
  const [option,setOption]=useState()

  const sidebar =useSelector((store)=> store.sidebar.sideValue)
  console.log(sidebar)

const getSearchdata = async ()=>{
  console.log(option)
  // const data =await fetch(YOUTUBE_SEARCH_VIDEO_API+searchText)
  // const json =await data.json()
  // setSuggestion(json[1])

}



  useEffect(()=>{
  const timer= setTimeout (()=> getSearchdata(),200)
  return ()=>{
  clearTimeout(timer)
 }
  // getSearchdata()
  },[searchText,option])
  return (
    <div className='grid grid-flow-col p-2'>

        <div className='flex col-span-2'> 
            <img  className='h-8 mx-2 my-2' src="data:image/png;base64," alt="" />
            <img className='h-8 mx-2 my-2' src="https://upload.wikimedia.org/wikipedia/commons/thumb/b/b8/YouTube_Logo_2017.svg/2560px-YouTube_Logo_2017.svg.png" alt="" />
        </div>

        <div className='col-span-8'>
          <input value={searchText} onChange={(e)=>{
            setSearchtext(e.target.value)
          }} className='border h-[40px] w-[500px] rounded-l-full p-2' placeholder= "Search Something !"  type="text" />
          <button className=' mt-2  h-[40px] w-[100px] border rounded-r-full bg-gray-200'>Search</button>
       {searchText && 
       <div className='fixed bg-gray-50 w-[500px] rounded-lg p-4'>
        <ul>
          {suggestion.map((e,index)=>{
            return  <li onClick={()=>{
              setOption(e)
              console.log("Clicked on",e)
            }} className='py-1 font-bold cursor-pointer hover:bg-gray-200 rounder' key={index}>🔍 {e}</li>
          })}
        </ul>
      </div>
      }
        </div>
        <div className='flex col-span-2'>

           <img src="data:image/svg+xml;base64," alt="" />
        </div>

    </div>
  )
}

export default Head